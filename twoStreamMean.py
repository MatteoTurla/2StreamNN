...
dist_dir = f'stats_mean/{cluster}_logits'
...
dataset = Dataset('../Dataset/aggregorio_skeletons_numpy/{}/test'.format(cluster), 
                        '../Dataset/aggregorio_videos_pytorch/{}/test'.format(cluster),
                        n_frames=nframes, downsample_pose=downsample_pose, downsample_video=downsample_video)
...
with torch.no_grad():
    for poses, videos, labels in loader:
        ...
        y_ = model_2stream.forward_mean(poses, videos)
